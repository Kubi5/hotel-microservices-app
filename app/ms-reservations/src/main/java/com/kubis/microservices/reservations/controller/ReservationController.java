...
private final ReservationService reservationService;
    ...
    public ResponseEntity<CollectionModel<EntityModel<ReservationModel>>> getCustomers(){
        List<ReservationModel> customers = reservationService.getReservations();

        List<EntityModel<ReservationModel>> reservationModels = customers.stream()
                .map(reservation -> {
                    Link selfLink = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(ReservationController.class).getCustomers())
        ...
        Link collectionLink = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(ReservationController.class).getCustomers())
    ...
    public ResponseEntity<EntityModel<ReservationModel>> getReservationById(@PathVariable("id") long id){
        Optional<ReservationModel> reservation = reservationService.getReservationById(id);

        if(reservation.isPresent()) {
            Link link = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(ReservationController.class).getReservationById(id))
                    .withSelfRel();
